-- Common query snippets for summaries, filtering, exporting

-- Total summary with optional date filtering
-- :startDate (inclusive), :endDate (inclusive)
SELECT
  COUNT(e.id) AS expense_count,
  COALESCE(SUM(e.amount), 0) AS total_amount
FROM expenses e
WHERE ( :startDate IS NULL OR e.created_at >= :startDate )
  AND ( :endDate   IS NULL OR e.created_at <= :endDate );

-- Category-wise summary with optional date filtering
SELECT
  c.id AS category_id,
...
LEFT JOIN expenses e
 ...
 AND ( :startDate IS NULL OR e.created_at >= :startDate )
 AND ( :endDate   IS NULL OR e.created_at <= :endDate )
GROUP BY c.id, c.name
ORDER BY total_amount DESC;

-- List expenses with filtering and pagination
-- :categoryId, :search (matches notes), :limit, :offset, :sortBy (created_at|amount), :sortDir (ASC|DESC)
SELECT
  e.id, e.amount, e.category_id, c.name AS category_name, e.notes, e.created_at
FROM expenses e
LEFT JOIN categories c ON c.id = e.category_id
WHERE ( :categoryId IS NULL OR e.category_id = :categoryId )
  AND ( :startDate IS NULL OR e.created_at >= :startDate )
  AND ( :endDate   IS NULL OR e.created_at <= :endDate )
  AND ( :search    IS NULL OR e.notes LIKE '%' || :search || '%' )
ORDER BY
  CASE WHEN :sortBy = 'amount' THEN e.amount END,
  CASE WHEN :sortBy = 'created_at' OR :sortBy IS NULL THEN e.created_at END
  COLLATE NOCASE
  /* direction controlled outside or via string build */
LIMIT :limit OFFSET :offset;

-- Export CSV: select headers-compatible resultset
SELECT
  e.id AS "ID",
  e.amount AS "Amount",
  c.name AS "Category",
...
Explanation: Add Node.js database helper (using sqlite3) to initialize schema, provide query helpers, and CSV export utility for backend usage.  
````write file="expense-tracker-pro-13571/expense_tracker_database/db.js"
/**
...
// Seed default categories from env or fallback list
async function seedDefaultCategories(db) {
  const row = await get(db, 'SELECT COUNT(*) AS cnt FROM categories;');
  if (row && row.cnt > 0) return;

  const envList = (process.env.EXPENSE_DEFAULT_CATEGORIES || 'Food,Travel,Utilities,Entertainment,Health,Education,Other')
    .split(',')
    .map(s => s.trim())
    .filter(Boolean);
...
async function listExpenses({
  ...
  const orderDir = allowedSortDir.includes(sortDir.toUpperCase()) ? sortDir.toUpperCase() : 'DESC';
